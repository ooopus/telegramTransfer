...
from .config import API_HASH as DEFAULT_API_HASH
from .config import API_ID as DEFAULT_API_ID
...
@click.option(
    "--to",
    help="目标用户/频道（支持ID、用户名或链接，例如：4639628806、@username 或 https://t.me/c/2257928502/2）",
)
@click.option(
    "--from",
    "source",
    help="源用户/频道（支持ID、用户名或链接，例如：4639628806、@username 或 https://t.me/c/2257928502/2）",
)
...
@click.option(
    "--keep-duplicates", is_flag=True, help="同步时保留远程重复文件，覆盖配置文件的设置"
)
def cli(
    operation: str,
    folder: str,
    ...
    # 如果命令行指定了 --keep-duplicates，则覆盖配置
    if keep_duplicates:
        from .config import config

        config.sync.delete_duplicates = False
        logger.info("命令行指定 --keep-duplicates，同步时将保留重复文件")
