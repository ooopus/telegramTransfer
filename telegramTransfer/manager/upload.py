...
class UploadMixin:
    ...
    async def verify_upload_status(
        self, folder_path: str, target: str
    ) -> Dict[str, Any]:
        """验证上传状态
        ...
        for msg in messages:
            ...
            try:
                ...
                if "original_path" in metadata:
                    ...
                    logger.debug(
                        f"找到远程文件: {metadata['original_path']}, 大小: {msg.file.size}"
                    )
        ...
        for rel_path, local_info in local_files.items():
            ...
            if remote_info["size"] != local_info["size"]:
                logger.debug(
                    f"文件 {rel_path} 大小不一致: 本地={local_info['size']}, 远程={remote_info['size']}"
                )
        ...
        return {"verified": verified, "missing": missing, "inconsistent": inconsistent}

    async def upload_folder(
        self, folder_path: str, target: str, caption_template: str = None
    ) -> Dict[str, Any]:
        """上传文件夹"""
        folder_path = os.path.expanduser(folder_path)
        ...
        if initial_verify["verified"]:
            logger.info(
                f"发现 {len(initial_verify['verified'])} 个文件已存在于目标位置"
            )
        ...
        # 根据配置对文件进行排序
        # 从全局配置的 transfer 部分获取 upload_order
        upload_order = config.transfer.upload_order

        # 计算所有文件的总大小
        total_size = sum(info["size"] for info in files_info.values())
        overall_progress = tqdm(
            total=total_size, desc="总体进度", unit="B", unit_scale=True, position=0
        )
        ...
        if verify_result["missing"] or verify_result["inconsistent"]:
            ...
            if verify_result["inconsistent"]:
                logger.warning(
                    f"发现 {len(verify_result['inconsistent'])} 个文件不一致:"
                )
    ...
    async def _upload_single_file(
        ...
        caption = (
            ...
            else caption_template.format(
                path=rel_path, size=info["size"], mtime=info["mtime"].isoformat()
            )
        )

        callback = self._create_progress_callback(
            f"上传 {rel_path}", info["size"], overall_progress=overall_progress
        )

        await self.client.upload_file(
            file_path, target, caption=caption, progress_callback=callback
        )
    ...
    def _sort_files(
        self, files_info: Dict[str, Dict], order: str
    ) -> List[Tuple[str, Dict]]:
        """根据指定顺序对文件进行排序
