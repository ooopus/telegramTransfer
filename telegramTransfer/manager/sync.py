...
class SyncMixin:
    ...
    async def _get_remote_state(self, target):
        ...
        for path, msg_list in duplicate_messages.items():
            if len(msg_list) > 1:
                ...
                if config.sync.delete_duplicates:
                    for old in sorted_msgs[1:]:
                        try:
                            await old["message"].delete()
                            logger.info(
                                f"已删除重复文件的旧版本: {path} (message_id={old['message_id']})"
                            )
                        except Exception as e:
                            logger.error(
                                f"删除重复文件失败: {path} (message_id={old['message_id']}): {str(e)}"
                            )
                else:
                    logger.info(f"检测到重复文件的旧版本，但根据配置未删除: {path}")
            else:
                # 只有一个版本，直接保存
                msg_info = msg_list[0]
                remote_state[path] = {
    ...
    def _calculate_diff(self, local_state, remote_state, sync_mode):
        ...
        return {
            ...
            "to_delete_local": local_files - remote_files
            if sync_mode == "from"
            else set(),
            "to_delete_remote": remote_files - local_files
            if sync_mode == "to"
            else set(),
        }
    ...
    async def _process_sync(
        ...
        if sync_mode == "from":
            await self._process_download_sync(
                folder_path, target, remote_state, diff, success, failed, deleted
            )
        else:
    ...
    def _save_sync_state(
        self, folder_path, target, sync_mode, local_state, remote_state
    ):
        """保存同步状态"""
    ...
    async def _process_upload_sync(
        self,
        folder_path,
        target,
        local_state,
        ...
        for msg in messages:
            try:
                ...
                try:
                    ...
                    logger.debug(
                        f"跳过非JSON格式消息: message_id={msg.id}, content={msg.message}"
                    )
                    continue
    ...
    async def _process_download_sync(
        self, folder_path, entity, remote_state, diff, success, failed, deleted
    ):
        """处理下载同步"""
        # 处理需要下载的文件
