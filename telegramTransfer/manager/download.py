...
class DownloadMixin:
    ...
    async def download_folder(
        self, source: str, download_path: str, limit: int = None
    ) -> Dict[str, Any]:
        """下载文件夹"""
        ...
        tqdm(total=total_size, desc="总体进度", unit="B", unit_scale=True, position=0)
    ...
    async def _download_single_file(
        self, message, download_path, downloaded, transfer_key
    ):
        """下载单个文件"""
        metadata = None
        if message.message:
        ...
        callback = self._create_progress_callback(
            f"下载 {file_name}", message.file.size
        )

        await self.client.download_file(message, file_path, progress_callback=callback)
