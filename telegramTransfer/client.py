...
# Removed json and tomli imports as config handling is now centralized
...
class TelegramTransferClient:
    ...
    def __init__(
        ...
        Args:
            ...
            session_name: 会话名称 (覆盖配置)
            api_id: Telegram API ID (覆盖配置)
            api_hash: Telegram API Hash (覆盖配置)
            session_dir: 会话文件存储目录 (覆盖配置)
        """
        # 使用传入的参数或全局配置中的值
        self.api_id = api_id if api_id is not None else API_ID
        self.api_hash = api_hash if api_hash is not None else API_HASH
        self.session_name = session_name if session_name is not None else SESSION_NAME

        # 设置会话文件存储目录
        # 优先使用传入的 session_dir，其次是全局配置的 SESSION_DIR
        self.session_dir = (
            Path(os.path.expanduser(session_dir)) if session_dir else SESSION_DIR
        )
    ...
    async def connect(self, force_new: bool = False) -> None:
        ...
        # 构建会话文件路径
        session_path = self.session_dir / f"{self.session_name}.session"
        # session_dir 已经在 __init__ 中创建
    ...
    async def download_file(
        self, message: Message, download_path: str, progress_callback: callable = None
    ) -> str:
        """下载单个文件
        ...
        path = await self.client.download_media(
            message.media, str(download_path), progress_callback=progress_callback
        )

        return path

    async def get_file_messages(
        self, dialog: Dialog, limit: int = None, reply_to: int = None
    ) -> List[Message]:
        """获取对话中的文件消息
    ...
    def scan_folder(
        self, folder_path: str, ignore_patterns: List[str] = None
        ...
        for path in folder_path.rglob("*"):
            if path.is_file():
                ...
                # 检查文件是否应该被忽略
                should_ignore = any(
                    pattern.match(rel_path) or pattern.match(path.name)
    ...
    def detect_changes(
        self, old_state: Dict[str, Dict[str, Any]], new_state: Dict[str, Dict[str, Any]]
    ) -> Dict[str, List[str]]:
        """检测文件变更
